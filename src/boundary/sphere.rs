...
macro_rules! impl_float_sphere {
    ($ty : ident) => {
        impl<const N : usize> Boundary<Cartessian<$ty, N>> for Sphere<Cartessian<$ty, N>>{
        ...
        impl Boundary<CartessianND<$ty>> for Sphere<CartessianND<$ty>>{
            ...
            fn find_intersect(&self, pos : &CartessianND<$ty>, movement : &CartessianND<$ty>) -> Option<CartessianND<$ty>> {
                ...
                let (x2, dx2, xdx, r) = (result.norm_l2_sqr(), movement.norm_l2_sqr(), result.inner_product(movement), self.radius);
                let t = (- xdx + (xdx * xdx + dx2 * (r * r - x2)).sqrt()) / dx2;
...
impl<T> Boundary<CartessianND<T>> for TaxiSphere<CartessianND<T>>
    where   T : Scalar + AbsDiffEq + PartialOrd + Neg<Output = T>,
...
mod test {
    ...
    fn test_sphere1d(){
        ...
        assert_abs_diff_eq!(range.find_intersect(&a, &movement).unwrap(), Cartessian1D::new([1f64]));
        assert_abs_diff_eq!(range.find_intersect_unsafe(&a, &movement).unwrap(), Cartessian1D::new([1f64]));
        ...
        assert_abs_diff_eq!(range.find_intersect(&a, &movement).unwrap(), CartessianND::new(vec![1f64]));
        assert_abs_diff_eq!(range.find_intersect_unsafe(&a, &movement).unwrap(), CartessianND::new(vec![1f64]));
    ...
    fn test_taxisphere2d(){
        ...
        a[1] = 2f64 / 3f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), Cartessian2D::new([-1f64/2f64.sqrt(); 2]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), Cartessian2D::new([-1f64/2f64.sqrt(); 2]));
        ...
        a[1] = 2f64 / 3f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), CartessianND::new(vec![-1f64/2f64.sqrt(); 2]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), CartessianND::new(vec![-1f64/2f64.sqrt(); 2]));
        ...
        movement[0] = 2f64; movement[1] = 0f64;
        b[0] = 1f64; b[1] = 0f64;
        assert_abs_diff_eq!(range.find_intersect(&a, &movement).unwrap(), b.clone());
        assert_abs_diff_eq!(range.find_intersect_unsafe(&a, &movement).unwrap(), b.clone());
    }
    ...
    fn test_taxisphere3d(){
        ...
        a[2] = 1f64 / 3f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), Cartessian3D::new([-1f64/3f64.sqrt(); 3]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), Cartessian3D::new([-1f64/3f64.sqrt(); 3]));

        a[0] = 1f64 / 3f64;
        a[1] = 2f64 / 3f64;
        a[2] = 0f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), Cartessian3D::new([-1f64/2f64.sqrt(), -1f64/2f64.sqrt(), 0f64]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), Cartessian3D::new([-1f64/2f64.sqrt(), -1f64/2f64.sqrt(), 0f64]));
        ...
        a[2] = 1f64 / 3f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), CartessianND::new(vec![-1f64/3f64.sqrt(); 3]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), CartessianND::new(vec![-1f64/3f64.sqrt(); 3]));

        a[0] = 1f64 / 3f64;
        a[1] = 2f64 / 3f64;
        a[2] = 0f64;
        assert_eq!(range.check_inclusion(&a), true);
        assert_abs_diff_eq!(range.normal_at(&a).unwrap(), CartessianND::new(vec![-1f64/2f64.sqrt(), -1f64/2f64.sqrt(), 0f64]));
        assert_abs_diff_eq!(range.normal_at_unsafe(&a), CartessianND::new(vec![-1f64/2f64.sqrt(), -1f64/2f64.sqrt(), 0f64]));
